...
const Kanva7 = () => {
  ...
  const Exit = () =>{
    axios.post("http://localhost:5000/eleminate",{email, IP, qno, way})
          .then(res => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/kan/add/07/data/07/${qno}?key=${reactkey}`)
    .then(resp => resp.json())
    ...
    fetch(`http://localhost:5000/validd/${email}?key=${reactkey}`)
      .then(res => res.json())
      .then(ipadr => setDatta(ipadr))

      fetch(`http://localhost:5000/lang/get/${email}?key=${reactkey}`)
      .then(res => res.json())
  ...
  const True = async (e) => {
    if(qno === "1"){
        const response = await fetch(`http://localhost:5000/way01/put/${wayy._id}`, {
          method: 'PUT',
          headers: {
  ...
  const Submitt1 = (e) => {
    e.preventDefault();
    axios.post("http://localhost:5000/kan/add/07/vall1/07",{value1, qno})
    .then(res => {
  ...
  const Submitt2 = (e) => {
    e.preventDefault();
    axios.post("http://localhost:5000/kan/add/07/vall2/07",{value2, qno})
    .then(res => {
      if(res.data.Status === "200") {
        True()
      }
  ...
  const Submitt3 = async (e) => {
    e.preventDefault();
    axios.post("http://localhost:5000/kan/add/07/vall3/07",{value3, qno})
    .then(res => {
      if(res.data.Status === "200") {
        True()
      }
    ...
    const Submitt4 = (e) => {
      e.preventDefault();
      axios.post("http://localhost:5000/kan/add/07/vall4/07",{value4, qno})
      .then(res => {
        if(res.data.Status === "200") {
          True()
        }else{
            ...
            <div key={i}>
              ...
              <div className='que-01-cont'>  
                <form onSubmit={Submitt1}>
                  <button onChange={(e) => setValue1(e.target.value)} onClick={resetTimer} title='option A' type='submit' className='hom-btn1' name='value' >{data.option1}</button>
                </form>
                <form onSubmit={Submitt2} >
                  <button onChange={(e) => setValue2(e.target.value)} onClick={resetTimer} title='option B' type='submit' className='hom-btn1' name='value' >{data.option2}</button>
                </form></div>}

                {dattta.way === "7" &&
                <div className='que-01-cont'>
                  <form onSubmit={Submitt3} >
                    <button onChange={(e) => setValue3(e.target.value)} onClick={resetTimer} title='option C' type='submit' className='hom-btn1' name='value' >{data.option3}</button>
                  </form>
                  <form onSubmit={Submitt4} >
                    <button onChange={(e) => setValue4(e.target.value)} onClick={resetTimer} title='option D' type='submit' className='hom-btn1' name='value' >{data.option4}</button>
                  </form>
                </div>}
                </div>}
                </div>}
