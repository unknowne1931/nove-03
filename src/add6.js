...
const MyForm6 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/data/06', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        window.location.href =`/question/06?email=${email}`
      } else {
...
const Lstpg = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?  
      <center>
        <button onClick={Lstpg} className='btn-admin-pnl-01'>Way Eng 06</button>
        {user.email === email ?
