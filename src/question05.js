...
const Ques05 = () => {
    ...
    useEffect(() =>{
      ...
      fetch(`http://localhost:5000/dataway05?key=${reactkey}`)
      .then(res => res.json())
  ...
  const Wayeng = () =>{
    ...
    return(
      <div className='qstn-cnt-01'>
        ...
        <center>
        <h1>Stawro</h1>
        <button onClick={Wayeng} className='btn-admin-pnl-01'>add eng 05</button><br />
        <span>Way 05</span>
          {datta.role ==="admin" ? 
          <div>
            ...
            {data.map((user,i) => {
              return(
                <div key={i} className='hom-01-brd'>
                  ...
                  <Link className='btn-01-01' to={`/delet5?email=${email}&id=${user._id}`} >Delete</Link>
                </div>
              )
            })}
          </div> :
