...
const Add4 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/kan/add/04/data/04', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        window.location.href = `/kand/04?email=${email}`
      } else {
...
const Way01kan = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?
      <center>
        <button onClick={Way01kan} className='btn-admin-pnl-01'>Way kan 04</button>
        {user.email === email ?   
