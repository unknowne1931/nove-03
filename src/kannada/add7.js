...
const Add7 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/kan/add/07/data/07', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        alert("okk");
        window.location.href = `/kand/07?email=${email}`
      } else {
...
const Way01kan = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?
      <center>
        <button onClick={Way01kan} className='btn-admin-pnl-01'>Way kan 07</button>
        {user.email === email ?   
