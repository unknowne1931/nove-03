...
const Add6 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/kan/add/06/data/06', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        window.location.href = `/kand/06?email=${email}`
      } else {
...
const Way01kan = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?
      <center>
        <button onClick={Way01kan} className='btn-admin-pnl-01'>Way kan 06</button>
        {user.email === email ?   
