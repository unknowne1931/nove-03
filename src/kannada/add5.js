...
const Add5 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/kan/add/05/data/05', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        window.location.href =`/kand/05?email=${email}`
      } else {
...
const Way01kan = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?
      <center>
        <button onClick={Way01kan} className='btn-admin-pnl-01'>Way kan 05</button>
