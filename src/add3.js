...
const MyForm3 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/data/03', {
        method: 'POST',
        headers: {
      ...
      if (response.ok) {
        window.location.href =`/question/03?email=${email}`
      } else {
...
const Lstpg = () =>{
  ...
  return (
    <div>
      ...
      {datta.role === "admin" ?  
      <center>
        <button onClick={Lstpg} className='btn-admin-pnl-01'>Way Eng 03</button>
        {user.email === email ?
