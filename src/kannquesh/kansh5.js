...
const Kansh5 = () => {
    ...
    useEffect(() =>{
      ...
      fetch(`http://localhost:5000/kan/add/05/dataway05?key=${reactkey}`)
      .then(res => res.json())
      ...
      <div className='qstn-cnt-01'>
        ...
        <center>
          ...
          <div>
            ...
            <h1>Stawro</h1>
            <span>Way 05</span><br />
            <button onClick={Way01kan} className='btn-admin-pnl-01'>Add Kannada 05</button>
            {data.map((user,i) => {
              return(
                <div key={i} className='hom-01-brd'>
                  ...
                  <Link className='btn-01-01' to={`/kand/del5?email=${email}&id=${user._id}`} >Delete</Link>
                </div>
              )
            })}
          </div> :
